...
// Size-class bin allocator (Unreal FMallocBinned style, simplified).
//
// Small allocations (size <= 512, alignment <= 16) are served from fixed-size
// bins carved out of page-aligned pages, so allocation/free is O(1) via an
// intrusive free list. The owning page (and therefore the bin) of any pointer
// is recovered by masking the pointer down to its page boundary and reading a
// page header - no per-block header overhead.
//
// Large or over-aligned allocations fall through to _aligned_malloc.
//
// Single-threaded: the engine is single-threaded at this stage, so the bins
// use no locking.
