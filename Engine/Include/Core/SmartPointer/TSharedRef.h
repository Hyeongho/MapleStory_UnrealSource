...
public:
    ...
    T& operator*()  const { return *m_SharedPtr; }
    T* operator->() const { return m_SharedPtr.Get(); }
    T* Get()        const { return m_SharedPtr.Get(); }

    int32 GetRefCount() const { return m_SharedPtr.GetRefCount(); }

    TSharedPtr<T> ToSharedPtr() const { return m_SharedPtr; }

    bool operator==(const TSharedRef& Other) const { return m_SharedPtr == Other.m_SharedPtr; }
    bool operator!=(const TSharedRef& Other) const { return m_SharedPtr != Other.m_SharedPtr; }
