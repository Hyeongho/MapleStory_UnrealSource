...
public:
        ...
        , m_pRefCountBlock(nullptr)
    {
        if (InPtr)
        {
            m_pRefCountBlock = static_cast<FRefCountBlock*>(
                FMemory::Malloc(sizeof(FRefCountBlock)));
    ...
    TSharedPtr(const TSharedPtr<U>& Other) noexcept
        : m_pElement(static_cast<T*>(Other.m_pElement))
        , m_pRefCountBlock(Other.m_pRefCountBlock)
    {
        if (m_pRefCountBlock)
            ++m_pRefCountBlock->m_SharedCount;
    ...
    ~TSharedPtr() { ReleaseRef(); }

    T* operator->() const { check(m_pElement != nullptr); return m_pElement; }
    T& operator*()  const { check(m_pElement != nullptr); return *m_pElement; }
    T* Get()        const { return m_pElement; }

    bool  IsValid()     const { return m_pElement != nullptr; }
    int32 GetRefCount() const { return m_pRefCountBlock ? m_pRefCountBlock->m_SharedCount : 0; }
    ...
    bool operator==(const TSharedPtr& Other) const { return m_pElement == Other.m_pElement; }
    bool operator!=(const TSharedPtr& Other) const { return m_pElement != Other.m_pElement; }
...
private:
    ...
    TSharedPtr(T* InPtr, FRefCountBlock* InBlock) noexcept
        : m_pElement(InPtr)
        , m_pRefCountBlock(InBlock)
    {}
