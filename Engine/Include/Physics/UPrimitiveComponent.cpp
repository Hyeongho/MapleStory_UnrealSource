...
{
	return m_bCollisionEnabled && Other.m_bCollisionEnabled
	       && (m_CollisionMask & CollisionChannelMask(Other.m_ObjectType))
	       && (Other.m_CollisionMask & CollisionChannelMask(m_ObjectType));
}
...
bool UPrimitiveComponent::Overlaps(const UPrimitiveComponent& Other) const
{
	...
	{
		const float Radius = static_cast<const UCircleCollision*>(this)->GetWorldRadius()
		                     + static_cast<const UCircleCollision&>(Other).GetWorldRadius();
