...
FFoothold::FFoothold(int32 Id, const FVector2D& Start, const FVector2D& End)
...
bool FFoothold::IsValid() const
{
	return m_Id >= 0 && _finite(m_Start.m_X) && _finite(m_Start.m_Y)
		&& _finite(m_End.m_X) && _finite(m_End.m_Y)
		&& FMath::Abs(m_End.m_X - m_Start.m_X) > FMath::KINDA_SMALL_NUMBER;
}
