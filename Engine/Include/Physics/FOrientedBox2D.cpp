...
FOrientedBox2D::FOrientedBox2D(const FVector2D& Center, const FVector2D& Extent, float Rotation)
	: m_Center(Center), m_Extent(FMath::Abs(Extent.m_X), FMath::Abs(Extent.m_Y))
{
...
{
	return m_Extent.m_X * FMath::Abs(m_AxisX.Dot(Axis))
		+ m_Extent.m_Y * FMath::Abs(m_AxisY.Dot(Axis));
}
...
FVector2D FOrientedBox2D::GetClosestPoint(const FVector2D& Point) const
{
	...
	return m_Center + m_AxisX * FMath::Clamp(Offset.Dot(m_AxisX), -m_Extent.m_X, m_Extent.m_X)
		+ m_AxisY * FMath::Clamp(Offset.Dot(m_AxisY), -m_Extent.m_Y, m_Extent.m_Y);
}
...
bool FOrientedBox2D::FindContact(
	const FOrientedBox2D& Other, FVector2D& Normal, float& Depth, float Tolerance) const
{
...
bool FOrientedBox2D::Sweep(
	const FOrientedBox2D& Other, const FVector2D& Delta, float& Time, FVector2D& Normal) const
{
	const FVector2D Axes[] = {m_AxisX, m_AxisY, Other.m_AxisX, Other.m_AxisY};
	const FVector2D Offset = m_Center - Other.m_Center;
...
bool FOrientedBox2D::Raycast(const FVector2D& Start, const FVector2D& End, float& Time,
	FVector2D& Normal, bool& bStartInside) const
{
